...
public sealed class RestoreHUD : MonoBehaviour
{
    ...
    private void Instruction(Transform p)
    {
        Image panel=Panel(p,new Vector2(.5f,.765f),new Vector2(850,76),new Color(.96f,.98f,.98f,.82f));
        instruction=Text(panel.transform,"Drag across the muddy areas",new Vector2(.5f,.5f),new Vector2(780,65),18,FontStyle.Normal,Ink);
    }
    ...
    private void Completion(Transform p)
    {
        overlay=new GameObject("Completion");overlay.transform.SetParent(p,false);RectTransform r=overlay.AddComponent<RectTransform>();r.anchorMin=Vector2.zero;r.anchorMax=Vector2.one;r.offsetMin=r.offsetMax=Vector2.zero;Image dim=overlay.AddComponent<Image>();dim.color=new Color(.02f,.07f,.08f,.62f);Image card=Panel(overlay.transform,new Vector2(.5f,.5f),new Vector2(820,500),new Color(.98f,1,1,.98f));Text(card.transform,"PERFECT RESTORATION",new Vector2(.5f,.60f),new Vector2(720,65),32,FontStyle.Bold,Ink);Text(card.transform,"Take a breath.  You did it.",new Vector2(.5f,.40f),new Vector2(680,60),19,FontStyle.Normal,new Color32(94,110,114,255));overlay.SetActive(false);
    }

    private void Update()
    {
        if(manager==null)return;float p=Mathf.Clamp01(manager.Progress);progress.text="RESTORATION  "+Mathf.RoundToInt(p*100)+"%";fill.fillAmount=p;current.text=manager.CurrentTool.Label();instruction.text=GetInstruction(manager.CurrentTool);int stage=Mathf.Clamp(manager.StageIndex,0,4);
        for(int i=0;i<5;i++){bool active=i==stage,unlocked=i<=manager.StageIndex;cards[i].color=active?Accent:unlocked?new Color(1,1,1,.15f):new Color(1,1,1,.07f);states[i].text=active?"ACTIVE":unlocked?"READY":"LOCKED";cards[i].GetComponent<Button>().interactable=unlocked&&!manager.IsComplete;}
        if(manager.IsComplete&&!overlay.activeSelf)overlay.SetActive(true);
    }
    ...
    private static Text Text(Transform p,string v,Vector2 a,Vector2 size,int f,FontStyle s,Color c){GameObject go=new GameObject("Label");go.transform.SetParent(p,false);RectTransform r=go.AddComponent<RectTransform>();r.anchorMin=r.anchorMax=a;r.anchoredPosition=Vector2.zero;r.sizeDelta=size;Text t=go.AddComponent<Text>();t.text=v;t.font=Resources.GetBuiltinResource<Font>("Arial.ttf");t.fontSize=f;t.fontStyle=s;t.alignment=TextAnchor.MiddleCenter;t.color=c;t.horizontalOverflow=HorizontalWrapMode.Wrap;t.verticalOverflow=VerticalWrapMode.Overflow;return t;}
}
