...
public sealed class RestoreHUD : MonoBehaviour
{
    ...
    private void Update()
    {
        TryInitialise();
        if(!built)return;
        float p=Mathf.Clamp01(manager.Progress);progress.text="RESTORATION  "+Mathf.RoundToInt(p*100)+"%";fill.fillAmount=p;current.text=manager.CurrentTool.Label();instruction.text=GetInstruction(manager.CurrentTool);int stage=Mathf.Clamp(manager.StageIndex,0,4);
