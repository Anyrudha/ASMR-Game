...
/// <summary>Detailed stylised sneaker surface plus a progressive alpha dirt mask.</summary>
public sealed class DirtLayer : MonoBehaviour
{
    ...
    private void CreateSneakerBase()
    {
        ...
        Color upper = new Color32(232, 238, 240, 255);
        Color upperShadow = new Color32(198, 209, 213, 255);
        Color panel = new Color32(53, 104, 121, 255);
        Color panelLight = new Color32(77, 132, 146, 255);
        Color sole = new Color32(249, 248, 242, 255);
        Color soleShadow = new Color32(211, 213, 207, 255);
        ...
        // Sole with a raised midsole and tread grooves.
        ...
        // Toe highlight and heel pull tab.
        Ellipse(p, 470, 188, 92, 52, new Color32(244, 247, 247, 255));
        RoundRect(p, 105, 284, 28, 72, 10, new Color32(43, 82, 95, 255));

        tex.SetPixels32(p); tex.Apply(false, true);
        GameObject shoe = new GameObject("Sneaker Surface"); shoe.transform.SetParent(transform, false);
    ...
    private void CreateDirtOverlay()
    {
        ...
        dirtTexture.filterMode = FilterMode.Bilinear; dirtTexture.wrapMode = TextureWrapMode.Clamp;
        ...
        // Heavier toe and sole grime for a clear before/after transformation.
        for (int i = 0; i < 45; i++) Stamp(Random.Range(420, 565), Random.Range(150, 225), Random.Range(5, 18), new Color32(82, 65, 51, 205));

        dirtTexture.SetPixels32(dirtPixels); dirtTexture.Apply(false, false);
        GameObject dirt = new GameObject("Dirt Mask"); dirt.transform.SetParent(transform, false);
    ...
    private void Stamp(int cx, int cy, int radius, Color32 color)
    {
        ...
        for (int x = Mathf.Max(0, cx - radius); x <= Mathf.Min(textureWidth - 1, cx + radius); x++)
        {
            ...
            if (a > alpha[idx]) { alpha[idx] = a; dirtPixels[idx] = new Color32(color.r, color.g, color.b, a); }
        }
    }
    ...
    private static void Polygon(Color32[] p, Vector2[] points, Color32 color)
    {
        ...
        foreach (Vector2 q in points) { minX = Mathf.Min(minX, q.x); maxX = Mathf.Max(maxX, q.x); minY = Mathf.Min(minY, q.y); maxY = Mathf.Max(maxY, q.y); }
    ...
    private void CreateBackground()
    {
        ...
        tex.SetPixels32(p); tex.Apply(false, true);
        GameObject shadow = new GameObject("Soft Shadow"); shadow.transform.SetParent(transform, false);
        ...
        r.transform.localPosition = new Vector3(0f, -1.75f, 0f); r.transform.localScale = new Vector3(2.1f, 0.8f, 1f); r.sortingOrder = -1;
    }
